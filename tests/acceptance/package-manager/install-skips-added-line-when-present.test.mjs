...
test("install does not reprint added packages when the store already has them", async (t) => {
  ...
  assert.match(second.stderr, /1 packages installed/);
  assert.equal(metaGets, 1);
  assert.equal(tarballGets, 1);
  ...
  assert.match(third.stderr, /1 packages installed/);
  assert.equal(tarballGets, 1);
  assert.equal(metaGets, 1);

  const importedAgain = await runProcess(process.execPath, [
    "--input-type=module",
    "-e",
    `import { hello } from "${pkgName}"; console.log(hello());`,
  ], { cwd: workspace });
  assert.equal(importedAgain.status, 0, importedAgain.stderr);
  assert.equal(importedAgain.stdout, "hello from fixture\n");
});
