...
test("why names the project that requires a direct dependency", async (t) => {
  ...
  const why = await runProcess(knot, ["why", pkgName], { cwd: workspace });
  assert.equal(why.status, 0, why.stderr);
  assert.match(why.stdout, /app/);
  assert.match(why.stdout, /knot-fixture-hello/);
});
