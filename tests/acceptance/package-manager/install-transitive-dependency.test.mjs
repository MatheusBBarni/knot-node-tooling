...
function startGraphRegistry() {
  ...
  const pkgs = {
    "knot-fixture-leaf": {
      version: "1.0.0",
      tarball: leafTar,
      integrity: sriSha512(leafTar),
    },
    "knot-fixture-mid": {
      version: "1.0.0",
      tarball: midTar,
      integrity: sriSha512(midTar),
    },
  };
  ...
  const server = http.createServer((req, res) => {
    ...
    for (const [n, pkg] of Object.entries(pkgs)) {
      const tarballPath = `/${n}/-/${n}-${pkg.version}.tgz`;
...
test("install materializes a transitive dependency so Node.js can import it", async (t) => {
  const registry = startGraphRegistry();
  ...
  assert.match(lock, /name knot-fixture-mid/);
  assert.match(lock, /name knot-fixture-leaf/);
  ...
  assert.equal(imported.stdout, "leaf\n");
});
