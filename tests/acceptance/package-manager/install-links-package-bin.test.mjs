...
test("install links a package bin so Node.js can run it", async (t) => {
  ...
  const bin = await runProcess(process.execPath, [
    path.join("node_modules", ".bin", "greet"),
  ], { cwd: workspace });
  assert.equal(bin.status, 0, bin.stderr);
  assert.equal(bin.stdout, "greet-ok\n");
});
