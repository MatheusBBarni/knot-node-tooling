...
const token = "knot-scoped-npmrc-token";

test("install sends host-scoped _authToken from project .npmrc", async (t) => {
  ...
  const workspace = await makeWorkspace({
    ...
    ".npmrc": `//${host}/:_authToken=${token}\n`,
  });
  t.after(() => removeWorkspace(workspace));

  const env = { ...process.env };
  delete env.KNOT_TOKEN;
