...
test("remove drops a dependency so Node.js can no longer import it", async (t) => {
  ...
  const removed = await runProcess(knot, ["remove", pkgName, "--registry", registryUrl], {
    cwd: workspace,
    timeoutMs: 60_000,
  });
  ...
  assert.equal(manifest.dependencies[pkgName], undefined);
