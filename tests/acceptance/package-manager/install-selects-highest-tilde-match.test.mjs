...
test("install selects the highest version that satisfies a tilde range", async (t) => {
  const registry = startRegistry({
    name: pkgName,
    versions: [
      { version: "1.2.0", files: pkgFiles("1.2.0", "v1.2.0") },
      { version: "1.2.5", files: pkgFiles("1.2.5", "v1.2.5") },
      { version: "1.3.0", files: pkgFiles("1.3.0", "v1.3.0") },
    ],
  });
  ...
  assert.match(lock, /version 1\.2\.5/);
  assert.doesNotMatch(lock, /version 1\.3\.0/);
  ...
  assert.equal(imported.stdout, "v1.2.5\n");
});
