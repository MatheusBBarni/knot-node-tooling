...
test("install strips Authorization when a metadata redirect changes origin", async (t) => {
  ...
  let destSawAuthorization = false;
  let destPort = 0;

  const dest = http.createServer((req, res) => {
    if (req.headers.authorization) {
      destSawAuthorization = true;
    }
      ...
      const body = JSON.stringify({
        ...
        versions: {
          "1.0.0": {
            ...
            version: "1.0.0",
            dist: {
              tarball: `http://127.0.0.1:${destPort}${tarballPath}`,
              integrity,
            },
          },
        },
  ...
  const origin = http.createServer((req, res) => {
    if (req.method === "GET" && req.url === `/${pkgName}`) {
      ...
      res.writeHead(302, {
        location: `http://127.0.0.1:${destPort}/${pkgName}`,
      });
      res.end();
      return;
    }
  ...
  const registryUrl = `http://127.0.0.1:${originPort}`;
  ...
  assert.equal(destSawAuthorization, false);
