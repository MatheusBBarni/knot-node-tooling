...
test("store status fails when a verified tarball is missing from the store", async (t) => {
  ...
  const ok = await runProcess(knot, ["store", "status"], { cwd: workspace, env });
  assert.equal(ok.status, 0, ok.stderr);

  await rm(path.join(home, ".knot", "cas"), { recursive: true, force: true });

  const missing = await runProcess(knot, ["store", "status"], { cwd: workspace, env });
