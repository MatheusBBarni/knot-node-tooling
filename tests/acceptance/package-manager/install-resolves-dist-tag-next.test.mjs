...
test("install resolves the next dist-tag to its tagged version", async (t) => {
  const registry = startRegistry({
    name: pkgName,
    distTags: { latest: "1.0.0", next: "2.0.0" },
    versions: [
      { version: "1.0.0", files: pkgFiles("1.0.0", "stable") },
      { version: "2.0.0", files: pkgFiles("2.0.0", "prerelease") },
    ],
  });
  ...
  assert.equal(imported.stdout, "prerelease\n");
});
