...
test("install resolves the latest dist-tag even when a higher version exists", async (t) => {
  const registry = startRegistry({
    name: pkgName,
    distTags: { latest: "1.0.0" },
    versions: [
      { version: "1.0.0", files: pkgFiles("1.0.0", "tagged") },
      { version: "2.0.0", files: pkgFiles("2.0.0", "newer") },
    ],
  });
  ...
  assert.doesNotMatch(lock, /version 2\.0\.0/);
  ...
  assert.equal(imported.stdout, "tagged\n");
});
